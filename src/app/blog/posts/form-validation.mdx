...
- define an **initialState** object that has a **success** boolean and an **errors** object
```ts
const initialState = {
  success: false,
  errors: {},
...
- use **useActionState** to handle the form action
```tsx
const [state, formAction] = useActionState(createTodo, initialState);
```

- use **state.errors** to show the error messages
```tsx
{state.errors?.title && <p>{state.errors.title}</p>}
{state.errors?.body && <p>{state.errors.body}</p>}
```

- use **state.message** to show the success message
```tsx
<p>{state.message}</p>
```

now, instead of manually setting the error state for each field, we can use **useActionState** to handle the form action and update the state based on the result.
